...
![Vivarium 수족관 창 — 실행 중인 AI 코딩 에이전트가 종별 물고기로 표시됩니다 (Claude 고래, Codex 문어, Gemini 해파리, Cursor 복어, OpenCode 돌고래, Copilot 바다거북)](docs/images/aquarium.png)

메뉴바 아이콘을 클릭하면 지금 돌아가는 에이전트 요약(팝오버)이 열립니다 — 상태·성장·산호 진행을 한눈에:

<img src="docs/images/popover.png" alt="메뉴바 팝오버 — 실행 중인 에이전트, 상태, 성장, 산호 진행 요약" width="340">
...
![여섯 종: Claude 고래, Codex 문어, Gemini 해파리, Cursor 복어, OpenCode 돌고래, Copilot 바다거북](docs/images/species.png)
...
- 🌊 **시스템 부하 = 물살** — 머신의 CPU·발열이 높을수록 물살이 세지고(물고기가 흔들리며 거슬러 헤엄) 물이 탁해집니다. 유휴 시 잔잔·투명, 저전력 모드에선 항상 잔잔
- 🔔 **대기 알림** — 에이전트가 턴을 끝내고 입력을 기다리면(오래 대기 시) 메뉴바 배지 + 옵션 로컬 알림으로 알려줍니다. 자리를 비웠을 때 놓치지 않도록
...
- **에이전트 대기 알림** — 기본 꺼짐(opt-in). 켜면 에이전트가 입력을 기다릴 때 로컬 알림을 보냅니다(켤 때 알림 권한을 요청). 수족관 창을 보고 있을 땐 알리지 않음
